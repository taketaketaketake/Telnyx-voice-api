    ...
    switch (eventType) {
      ...
      case 'call.hangup':
        ...
        if (callId) {
          ...
          // Fetch transcript after delay to allow Telnyx to generate it
          ...
          setTimeout(async () => {
            try {
              ...
              if (transcript && transcript.transcript_text) {
                ...
                console.log('⚠️ Telnyx may still be processing. Transcript will be fetched from API when user requests it.');
              }
            } catch (error) {
              console.log('❌ Could not fetch transcript for call:', callId, '-', error.message);
              console.log('⚠️ Transcript will be fetched from Telnyx API when user requests it.');
            }
          }, 60000); // Wait 60 seconds for transcript to be ready
        }
        break;
